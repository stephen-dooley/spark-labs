...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.mapValues(list).map(lambda x: (x[0], sum(x[1])));
...
def wordCount(wordListRDD):
    ...
    """
    from operator import add;
    wordAndCount = wordListRDD.map(lambda x: (x, 1));
    wordCount = wordAndCount.reduceByKey(add);    
...
print removePunctuation("Hi!, you are hello .")
print removePunctuation(" No under_score!. , 33  dd90ur0^˙^ø˙ø∑∑  ∑∑øo;.\. r34werjo  wojjwnn   ")
...
# TODO: Replace <FILL IN> with appropriate code
shakeWordsRDD = shakespeareWordsRDD.filter(lambda x: filter(None, x))
...
# #### ** (4f) Count the words **
# #### We now have an RDD that is only words. Let's apply the `wordCount()` function to produce a list of word counts. We can view the top 15 words by using the `takeOrdered()` action; however, since the elements of the RDD are pairs, we need a custom sort function that sorts using the value part of the pair.
